...
async function downloadSessionData(config) {
    try {
...
async function start() {
    ...
    const { Handler, Callupdate, GroupUpdate } = await import('./data/index.js');
    const { default: pkg } = await import('./lib/autoreact.cjs');
    const { emojis, doReact } = pkg;

    try {
        // Try downloading session if SESSION_ID provided
        await downloadSessionData(config);
